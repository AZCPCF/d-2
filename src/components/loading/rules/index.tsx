...
export default function RulesPageLoading({
  ...
  return (
    <main className="p-10 max-md:p-4 mx-auto bg-gray-100 min-h-screen">
      <section
        className="w-full bg-background rounded-xl shadow-sm mb-6"
        role="status"
        aria-label="در حال بارگذاری قوانین سایت"
        aria-busy="true"
      >
        ...
        <div className="space-y-4 px-6 py-4">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="h-4 bg-gray-200 rounded w-full loading"
              aria-hidden="true"
            />
          ))}
        </div>
      ...
      <div className="flex items-center gap-2 w-full p-4 text-gray-200 bg-white rounded-lg text-xl">
        <TbLicense aria-hidden="true" focusable="false" />
        <div
          className="h-5 w-48 bg-gray-50 rounded loading"
          aria-hidden="true"
        />
      </div>
    </main>
